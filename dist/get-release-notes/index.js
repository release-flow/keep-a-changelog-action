...
function getPrepareReleaseOptions() {
    ...
    if (!isValidReleaseType(releaseType)) {
        core.setFailed(`Input 'release-type' has an invalid value '${releaseType}'. The value must be one of: major, premajor, minor, preminor, patch, prepatch, or prerelease`);
        return;
    }
...
/* harmony default export */ const preprocessor = (attacher);
...
;// CONCATENATED MODULE: ./lib/get-release-notes.js
...
async function processChangelog(file, options) {
    ...
    const updated = await remark()
        ...
        .use(preprocessor)
        .use(assert)
        .use(extract_release_notes, options.version)
...
async function run() {
    ...
    try {
        ...
        lib_core.setOutput('release-notes', result);
...
void run();
//# sourceMappingURL=get-release-notes.js.map
})();
